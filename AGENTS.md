...
`@vllnt/convex-permissions` is a Convex component: Role-based access control as a Convex component — typed, sandboxed, runtime-editable roles and grants keyed by opaque refs It follows the vllnt Component
...
├── client/
│   ├── index.ts           # Permissions client class (consumer-facing API)
...
└── component/
    ...
    └── convex.config.ts    # defineComponent("permissions")
```
